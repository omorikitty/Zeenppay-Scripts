...
def get_glowing_colors(self) -> None:
    try:
        ...
        babase.screenmessage("Error: initializing the code", color = (1, 0.3, 0))
...
def update_from_profile(self) -> None:
    ...
    if self._profilename == '_edit':
        ...
        if self._profilename[0] in self.glowDict:
            cM = self.glowDict[self._profilename[0]][0]
            hM = self.glowDict[self._profilename[0]][1]
            stabilizeC = int(self.glowDict[self._profilename[0]][2]) > 0
            stabilizeH = int(self.glowDict[self._profilename[0]][3]) > 0
...
def enable():
    ...
    _lobby.Chooser.get_glowing_colors = get_glowing_colors
    _lobby.Chooser.update_from_profile = update_from_profile
