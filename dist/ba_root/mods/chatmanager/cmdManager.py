...
class CommandManager:
    ...
    def process_command(self, command, client_id, account_id=None):
        ...
        # Verifica si el comando esta en un periodo de espera
        if (
            client_id in self.command_cooldowns.get(command, {})
            and current_time - self.command_cooldowns[command][client_id]
            < self.cooldown_duration
        ):
            remaining = int(
                self.cooldown_duration
                - (current_time - self.command_cooldowns[command][client_id])
            )
            sendmsg(
        ...
        if account_id is not None and is_admin(account_id):
            if self.command_count[client_id] > self.admin_bypass_threshold:
